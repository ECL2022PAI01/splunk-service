...
# Example: ./build-splunk-service.sh -d ../. -u dockerUsername -v 0.1.0 -l
...
function checking_pod_termination() {
	...
	while true; do
		...
		pod_name=$(kubectl -n keptn get pods | grep splunk-service | awk '{print $1}')
...
function checking_pod_running() {
	...
	while true; do
		# Get the current output from the kubectl command
		pod_state=$(kubectl -n keptn get pods | grep splunk-service | awk '{print $3}')
...
helm upgrade --install -n keptn splunk-service $chartName --set splunkservice.existingSecret=splunk-service-secret
